...
12. **And** a Maestro automation test flow is created in `.maestro/flows/` that:
    ...
    - Enters scores for a player to reach a score that would exceed 50 (e.g., player at 48, enters 5)
    - Verifies the system automatically detects the score exceeds 50
    - Verifies the player's score is automatically reset to 25
    - Verifies clear visual feedback is provided (score animation, orange indicator, clear message)
    - Verifies strong haptic feedback is triggered
    - Verifies the penalty enforcement happens in real-time without perceptible delay
    - Verifies all players can see the penalty was applied on the main game screen
