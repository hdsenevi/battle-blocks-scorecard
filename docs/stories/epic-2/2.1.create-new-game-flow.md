...
9. **And** a Maestro automation test flow is created in `.maestro/flows/` that:
   - Launches the app
   - Verifies the "Start New Game" button is visible on the home screen
   - Taps the "Start New Game" button
   - Verifies navigation to the new game setup screen
   - Verifies the player addition interface is visible
