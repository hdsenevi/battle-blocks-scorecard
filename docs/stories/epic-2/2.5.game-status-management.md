...
- Database status field verified:
  ...
  - Valid values: "active", "completed", "paused"
...
- Status transition logic implemented:
  ...
  - active → paused: Status maintained as "active" (paused status not currently used)
- Status filtering implemented:
  - listActiveGames() used for resume functionality (home screen, game selection)
  - listCompletedGames() available in database service for history
