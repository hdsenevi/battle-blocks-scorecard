...
- Maestro automation test flow created:
  - Flow file: `.maestro/flows/notcompleted-game-state.yaml`
  - Tests pause on navigation back, notcompleted transition, and resume list exclusion
  - Verifies that paused games are marked as notcompleted when starting a new game
  - Verifies that notcompleted games are excluded from resume lists
- Linting passes for all files

### File List
- `src/database/types.ts` - GameStatus type updated to include 'notcompleted' (already existed, verified)
- `src/database/schema.sql` - CHECK constraint updated to include 'notcompleted' (already existed, verified)
- `src/services/database.ts` - Schema creation and listPausedGames() function (already existed, verified)
- `src/app/game/[id]/index.tsx` - Pause on navigation back logic (already existed, verified)
- `src/app/game/new.tsx` - Mark paused games as notcompleted when starting new game (already existed, verified)
- `src/components/game/ScoreEntryModal.tsx` - Prevent score entry for notcompleted games (already existed, verified)
- `src/app/(tabs)/index.tsx` - Resume logic updated to include paused games (already existed, verified)
- `src/app/game/select.tsx` - Resume logic updated to include paused games (already existed, verified)
- `.maestro/flows/notcompleted-game-state.yaml` - Maestro E2E test flow for notcompleted game state (NEW)
