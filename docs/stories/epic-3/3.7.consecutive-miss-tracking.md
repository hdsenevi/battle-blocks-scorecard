...
9. **And** a Maestro automation test flow is created in `.maestro/flows/` that:
    ...
    - Enters a score of 0 (miss) for a player
    - Verifies the player's consecutive_misses counter is incremented
    - Enters another miss (score of 0) for the same player
    - Verifies the consecutive_misses counter increments again
    - Enters a non-zero score for the player
    - Verifies the consecutive_misses counter is reset to 0
    - Verifies the tracking happens automatically without user intervention
