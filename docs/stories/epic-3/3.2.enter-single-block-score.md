...
11. **And** a Maestro automation test flow is created in `.maestro/flows/` that:
    ...
    - Selects "Single Block" option
    - Enters a valid block number (e.g., 12) and submits
    - Verifies the score is calculated correctly (12 points)
    - Verifies the player's score updates on the main game screen
    - Verifies the score entry interface closes after submission
    - Attempts to enter invalid input (negative, non-numeric) and verifies error handling
