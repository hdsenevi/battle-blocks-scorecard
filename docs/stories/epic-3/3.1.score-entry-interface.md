...
9. **And** players can only score once per round (single score per round rule)
10. **And** if a player has already scored this round, score entry is blocked with a clear message explaining they must wait until the next round
11. **And** the interface shows which players have already scored this round (via PlayerCard "Scored This Round" badge)
12. **And** a Maestro automation test flow is created in `.maestro/flows/` that:
    ...
    - Verifies that players can only score once per round
    - Verifies that attempting to score again shows appropriate message
...
- [x] Implement accessibility features (AC: 6)
  ...
  - [x] Support system font scaling (via NativeWind Text component with className)
...
- [x] Implement single score per round rule (AC: 9, 10, 11)
  - [x] Check `playersWhoScoredThisRound` set before allowing score entry
  - [x] Block score entry if player already scored this round
  - [x] Display clear message explaining round restriction
  - [x] Show "Scored This Round" badge on PlayerCard for players who have scored
...
- **Round Management**: Single score per round rule enforced via `playersWhoScoredThisRound` set
- **Round State**: GameContext tracks which players have scored in current round
...
- Single score per round rule implemented:
  - ScoreEntryModal checks `playersWhoScoredThisRound` set before allowing entry
  - Players who already scored are blocked with clear message
  - PlayerCard shows "Scored This Round" badge for players who have scored
  - ADD_SCORE action automatically adds player to `playersWhoScoredThisRound` set
