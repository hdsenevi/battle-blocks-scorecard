...
9. **And** a Maestro automation test flow is created in `.maestro/flows/` that:
    ...
    - Enters multiple scores for different players
    - Accesses the score history (via button, swipe, or menu)
    - Verifies all score entries are displayed with player name, score value, entry type, and timestamp
    - Verifies entries are displayed in chronological order
    - Verifies the history is scrollable if there are many entries
