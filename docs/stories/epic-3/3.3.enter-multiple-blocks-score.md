...
12. **And** a Maestro automation test flow is created in `.maestro/flows/` that:
    ...
    - Selects "Multiple Blocks" option
    - Enters a valid block count (e.g., 3) and submits
    - Verifies the score is calculated correctly (3 points)
    - Verifies the player's score updates on the main game screen
    - Verifies the distinction between single block and multiple blocks is clear in the UI
    - Verifies the score entry interface closes after submission
